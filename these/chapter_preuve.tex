...
\begin{notation}
  ...
  \item \comp{$\Gamma$}{env} calcule a nouvel environnement résultant de
...
\begin{notation}
  ...
  \item $env_{i_n}$ denotes the environement obtained after computation of the
    instruction labeled $i_n$ (\comp{$i_n$}{env} = $env_{i_n}$)
  \end{itemize}
\end{notation}
...
\begin{lemma}\label{lem:term-correct}
  ...
  \eval{$t'$}{\comp{$\Gamma$}{env}} = \eval{$t$}{env}.
\end{lemma}
...
{\myinference[CST]
  {}
  { (v : \mathbb{Z}) \rulearrow (\Zinit\underline{\mathtt{x = v}}, x) }
}
...
\begin{lemma}\label{lem:pred-correct}
  ...
  \eval{$p'$}{\comp{$\Gamma$}{env}} = \eval{$p$}{env}.
\end{lemma}
...
{\myinference[AND]
  { p_1 \rulearrow (\Gamma_1, e_1) \\
    p_2 \rulearrow (\Gamma_2, e_2) }
  { \mathtt{p_1\ \&\&\ p_2} \rulearrow (\Gamma, var) }
}~\\
...
{\myinference[FORALL]
  {
    (t_1 : \mathbb{Z}) \rulearrow (\Gamma_1, e_1) \\
    (t_2 : \mathbb{Z}) \rulearrow (\Gamma_2, e_2) \\
    p \rulearrow (\Gamma_3, e)
  }
  { \mathtt{\backslash forall\ integer\ id;\ t_1 <= id < t_2 ==> p}
    \rulearrow (\Gamma, var) }
}~\\
