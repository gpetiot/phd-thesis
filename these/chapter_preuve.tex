...
\begin{figure}[bt]
  \begin{tabular}{p{4.2cm} p{.4cm} p{9cm} p{2cm}}
    ...
    & \eqlabel{C-set} \\
    \comp{\lstinline'lval = f(e1, ..., eN);'}{\env}
    &=&\env[\lstinline'lval' $\mapsto$ \eval{\lstinline'__ret'}{(\compf{\lstinline'f(e1, ..., eN)'}{\env})}]
    & \eqlabel{C-fct1} \\
...
\begin{figure}[bt]
  \begin{tabular}{p{.5cm} p{12cm} p{2cm}}
    \multicolumn{3}{l}{
      \compf{\lstinline'/*@ requires p1; assigns X; ensures p2; */ f(e1, ..., eN)' $\bopen A \bclose$}{\env}
    } \\
    ...
    =& \errorenv si $\lnot$ \eval{\lstinline'p2'}{(\compi{$A$}{\env})}
    & \eqlabel{F-4} \\
    =& \compi{$A$}{\env} sinon & \eqlabel{F-5} \\
  \end{tabular}
...
La sémantique dénotationnelle des fonctions est donnée en
figure~\ref{fig:sem-fct}.
La fonction $\mathcal{F}$ calcule un nouvel environnement à partir d'une
...
d'appel.
Nous traitons les deux cas de manière séparée dans les deux sous-sections
suivantes.
...
(figure~\ref{fig:sem-fct}) est gérée par les lignes 3--4 du code instrumenté :
...
La règle \eqlabel{f-2} ne s'applique par à la fonction sous vérification.
La règle \eqlabel{f-3} est gérée par les lignes 5--7 et 10--12 : \errorenv est
produit si $\exists x. x \in G-X.$
(\eval{\lstinline'x'}{(\compi{$A$}{\env})}) $\ne$ (\eval{\lstinline'x'}{\env}),
où $G-X = \{\mbox{\lstinline'x1'},\ldots,\mbox{\lstinline'xN'}\}$.
La règle \eqlabel{f-4} est gérée par les lignes 13--14 : \errorenv est produit
...
\textsc{$\alpha$-check-assigns} et \textsc{$\alpha$-check-post} de la section
précédente.
...
2 et les lignes 13--14 résultent de la traduction de la postcondition ligne 3.
La règle \eqlabel{f-1} de la sémantique dénotationnelle des fonctions
(figure~\ref{fig:sem-fct}) ne s'applique par aux fonctions appelées.
La règle \eqlabel{f-2} est gérée par les lignes 3--4 du code instrumenté :
\errorenv est produit si $\lnot$ \eval{\lstinline'p1'}{\env}.
Les autres règles sont traduites de manière similaire à la section précédente.
La règle \eqlabel{f-3} est gérée par les lignes 5--7 et 10--12 : \errorenv est
produit si $\exists x. x \in G-X.$
(\eval{\lstinline'x'}{(\compi{$A$}{\env})}) $\ne$ (\eval{\lstinline'x'}{\env}),
où $G-X = \{\mbox{\lstinline'x1'},\ldots,\mbox{\lstinline'xN'}\}$.
...
L'objectif du chapitre était de fournir une preuve de correction pour les règles
de traduction des annotations et des termes et prédicats \eacsl qui les
composent.

Grâce à cette preuve, nous pouvons assurer l'absence de faux positifs et de faux
négatifs lors de la génération de tests sur le programme traduit, ainsi que
la non-modification par le code généré des variables du programme d'origine.

La preuve de ces propriétés est fournie de manière formelle pour les assertions
