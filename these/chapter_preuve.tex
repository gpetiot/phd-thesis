...
\begin{figure}
  \begin{tabular}{p{4.2cm} p{.4cm} p{9cm} p{2cm}}
    ...
    & \eqlabel{C-while-3} \\
    & = & \errorenv si $\lnot$ \eval{p}{(\comp{A}{\env})}
    & \eqlabel{C-while-4} \\
    & = & \errorenv si (\eval{t}{(\comp{A}{\env})}) $\ge$ (\eval{t}{\env})
    & \eqlabel{C-while-5} \\
    & = & \errorenv si
...
$\equiv$ \comp{$I \concat$ \lstinline'fassert(e);'}{\env} = \errorenv
...
\begin{proof}
  ...
  \commentGP{Prouver que \comp{$I$}{\env} $\neq$ \errorenv.}
