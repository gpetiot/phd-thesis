...
impactées dans l'environnement \compi{$I$}{\env}.
Ces deux propriétés sont vérifiées par la traduction de chaque terme.
...
\eval{\lstinline'x'}{(\compi{$(l, \Zinit$\underline{\lstinline'x = v;'}$)$}{\env})}
...
\begin{proof}
  ...
  La variable \lstinline'x' étant fraîche dans l'environnement, la seconde
...
Les variables qui sont déjà dans l'environnement initial \env ne sont pas
impactées dans l'environnement \compi{$I$}{\env}.
Ces deux propriétés sont vérifiées par la traduction de chaque prédicat.
Elles nous permettent de prouver les deux théorèmes~\ref{th:error}
et~\ref{th:inclusion}.
...
\begin{proof}
  ...
  Nouvelle hypothèse : \eval{\lstinline'p'}{\env} = \textit{true} \eqlabel{h3}

  \commentGP{TODO}

  (2) Cas où l'assertion est invalide :

  Nouvelle hypothèse : \eval{\lstinline'p'}{\env} = \textit{false} \eqlabel{h4}
