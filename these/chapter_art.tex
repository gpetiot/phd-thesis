...
statiques et analyses dynamiques, ou ayant pour objectif d'aider la vérification
déductive.
Nous présentons leurs forces et leurs faiblesses puis concluons en mettant en
avant ce que nous souhaitons apporter à ces travaux.
...
techniques de vérification sur différentes propriétés d'une même fonction.
...
par \pex.
%% Cette étape est semblable à l'instrumentation
...
La plateforme \textsc{staRVOOrS} \cite{Ahrendt/FM15} propose une vérification en
...
l'exécution avec \textsc{Larva} afin de vérifier les propriétés portant sur le
flot de contrôle.
Cette méthode repose sur \textsc{ppDATE}, un langage de spécification encodant
les propriétés du flot de contrôle à base d'automates, où les états de
l'automate sont augmentés par des pré- et postconditions (à la \jml) afin
d'encoder les propriétés portant sur les données du programme.
...
Une précision plus grande peut être obtenue en analysant statiquement les
branches d'un arbre de preuve.
...
suffisamment forts et elle n'est automatique que si le proveur peut décider la
(non-)satisfiabilité d'une formule logique de premier ordre exprimant la
...
Cette méthode suggère des modification à apporter au code et à la spécification 
...
prouveurs et sont donc rarement vérifiées.
Néanmoins, les vérifier permet de s'assurer de la consistance d'une
axiomatisation.
...
d'une axiomatisation invalide (inconsistante), ce qui est complémentaire à nos
...
\cite{Muller/FM11} propose d'étendre un validateur d'assertions à l'exécution
({\em runtime assertion checker}) afin de l'utiliser comme débogueur
({\em debugger}) afin de comprendre les contre-exemples d'échecs de preuve
complexes.
...
outils de vérification tels \boogie.
...
Nous poursuivons les travaux précédents afin d'aider la vérification déductive
par la génération de contre-exemples.
La technique originale de diagnostic des échecs de preuve que nous présentons
dans les chapitres suivants donne un résultat plus précis que les travaux
présentés dans ce chapitre.
En particulier, nous affirmons que les contre-exemples générés par notre méthode
permettent de mieux identifier la raison des échecs de preuve qu'un
contre-exemple extrait à partir d'un prouveur.
...
fournissant automatiquement un retour précis à l'utilisateur quant à la raison
des échecs de preuve, n'a pas été étudiée, implémentée et expérimentée
auparavant.
