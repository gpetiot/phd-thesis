...
\scriptsize{
  {\myinference[$\tau$-old\hspace{-2mm}{}]
    {}
    {(l, \mathtt{\backslash old(x)}:ctype) \rulearrow (\emptyset, \mathtt{old_x})}
  }
  ...
  {\myinference[$\tau$-old-val\hspace{-2mm}{}]
    ...
    {(l, \mathtt{\\old(*(x+t))}:ctype) \rulearrow (I,\mathtt{*(old_val_x+e)})}
  }
  \vspace{-7mm}

  {\myinference[$\tau$-res\hspace{-2mm}{}]
    {}
    {(l, \mathtt{\\result}:ctype) \rulearrow (\emptyset, \mathtt{res})}
  }
...
%\lstinline|\old| applied on a complex term is
%propagated to simpler terms,
%and we end up with \lstinline|\old| only applied to identifiers.
%Notice that the declaration of variables like \lstinline|old_x| and
%\lstinline|old_val_x| is
%systematically made at the beginning of each function as explained in
%Sec.~\ref{sec:principles}.
...
\textsc{Frama-C}. 
%We consider it is always called \lstinline|res| in the
%normalized form.
...
{\myinference[$\tau$-sum]
  ...
  {
    \splitfrac
        {(l, (\mathtt{\\sum(t1, t2, \\lambda integer i; t3)}):\mathbb{Z})
          \rulearrow}
        {
          (I_1 \concat I_2
          \concat
...
{\myinference[$\tau$-numof]
  ...
  {
    \splitfrac
        {(l, (\mathtt{\\numof(t1, t2, \\lambda integer i; t3)}):\mathbb{Z})
          \rulearrow}
        {
          (I_1 \concat I_2
          \concat
...
\scriptsize{
  ...
  {\myinference[$\pi$-and]
    ...
    {(l, \mathtt{p1 \&\& p2})
      \rulearrow
      (I_1 \concat (l, \mathtt{int var_n = e1; if(var_n) \{})
      \concat I_2
      \concat (l, \mathtt{var_n = e2; \}}), \mathtt{var_n})
    }
  }
  {\myinference[$\pi$-true]
    {}
    {(l, \mathtt{\\true}) \rulearrow (\emptyset, \mathtt{1})}
  }
  ...
  {\myinference[$\pi$-false]
    {}
    {(l, \mathtt{\\false}) \rulearrow (\emptyset, \mathtt{0})}
  }
  ...
  {\myinference[$\pi$-impl]
    ...
    {(l, \mathtt{p1 ==> p2})
      \rulearrow
      (I_1 \concat (l, \mathtt{int var_n = 1; if(e1) \{})
      \concat I_2
      \concat (l, \mathtt{var_n = e2; \}}), \mathtt{var_n})
    }
  }
  ...
  {\myinference[$\pi$-if]
      ...
      (l, p_3) \rulearrow (I_3, e_3)}
    {
      ...
      \underline{\mathtt{e1}\Zclear\ \mathtt{ != 0}}
      \mathtt{) \{})
      \concat I_2 \concat (l, \mathtt{var_n=e2; \} else \{})
      \concat I_3 \concat (l, \mathtt{var_n=e3; \}}),
      \mathtt{var_n})
    }
  }
  \vspace{-0.5mm}
...
\scriptsize{
  ...
  {\myinference[$\pi$-valid-range]
    {(l, t_1:ptr) \rulearrow (I_1, e_1) \\
      ...
      (l, t_3:int) \rulearrow (I_3, e_3)}
    {
      (l, \mathtt{\\valid(t1+(t2..t3))}) \rulearrow
...
\scriptsize{
  {\myinference[$\pi$-exists]
    ...
    {
      \splitfrac
          {(l, \mathtt{\\exists integer i; t1 <= i < t2 \&\& p}) \rulearrow
          }
          {
...
\begin{figure}[tb]
  ...
  \begin{tikzpicture}[align=left]
    ...
    %% %% dots
    %% \node (dots) [right=1mm of box-g1] {...};

    %% %% gn
    %% \node (box-gn) [func,right=.2cm of dots] {
    %%   \lstinline|fassert(gn_precond(...));|\\ ... \\ ...};
    %% \path (box-gn)+(0,3em) node (pre-gn) [pre] {\{Pre\}};
    %% \path (box-gn)+(0,-3em) node (post-gn) [post] {\{Post\}};
    %% \node [below=.1cm of post-gn] {callee $g_n$};
    %% %% gn_precond
    %% \node (gn-precond) [precond,above=.6cm of pre-gn] {};
    %% \node [below=.1cm of gn-precond] {\scriptsize $g_n\_precond$};
    %% %% link between gn and gn_precond
    %% \path (box-gn.west)+(-.1,2.4) node (anchor-pre-gn) {};
    %% \path (box-gn.west)+(1.3,2.4) node (anchor-pre-gn-2) {};
    %% \path [draw, -] (pre-gn.west) -| (anchor-pre-gn.west);
    %% \path [draw, ->] (anchor-pre-gn.west)
    %% -- node [above] {} (anchor-pre-gn-2.east);
    %% %% gn_post
    %% \path (box-gn.west)+(-.1,-.4) node (anchor-post-gn) {};
    %% \path (box-gn.west)+(0,-.4) node (anchor-post-gn-2) {};
    %% \path [draw, -] (post-gn.west) -| (anchor-post-gn.west);
    %% \path [draw, ->] (anchor-post-gn.west)
    %% -- node [above] {} (anchor-post-gn-2.east);
  \end{tikzpicture}
  \vspace{-8mm}
...
being translated.
%, it
%returns $1$ or $0$ with respect to the value of the precondition of the
%considered function. Thus this 
...
%adding the constraints of the precondition to
%the path predicate of the function under test.
    ...
    }
    {
      \splitfrac
          ...
          }
          {
            ...
            \concat I_2
            \concat (BegIter_l,
            \underline{\Zinit \mathtt{old_variant = e2} \Zclear}
            \semicolon
            )
            \concat I_3
            \concat(EndIter_l,
            ...
            \mathtt{ \&\& }\
            \underline{\mathtt{e3}\Zclear\mathtt{< old_variant} \Zclear}
            \mathtt{);})
          }
    }
  }
...
%point out some
%specificities of \textsc{PathCrawler} taken into consideration in
%our implementation of the translation rules in \textsc{StaDy}.

%\textbf{Similarities and differences with Runtime Assertion Checking.}
%\paragraph{Similarities and differences with Runtime Assertion Checking}
%both ensue from our formalization. 
...
%The presented translation rules are very similar to the translation
%for runtime assertion checking in \textsc{Frama-C}
%and will constitute the starting point for their complete formalization.
...
defined by \textsc{PathCrawler} that return the value of validity of a pointer
...
tool. \textsc{PathCrawler} offers dedicated builtin support 
...
by running  the \textsc{Frama-C/RTE} plugin~\cite{Frama-C} on the instrumented code
...
are not treated by \textsc{RTE}, but they can be easily prevented 
