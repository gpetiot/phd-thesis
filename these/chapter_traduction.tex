...
\section{Similarités et Différences entre le Test et la Validation à
  l'Exécution}
...
deux cas.
De plus, la plupart des règles définies dans les sections~\ref{sec:term},
\ref{sec:pred} et \ref{sec:annot} sont autant valable pour la validation à
l'exécution que pour le test.
...
programmes avec des valeurs d'entrées incorrectes, pour lesquelles le bon
...
Par exemple, le cast d'un term $t$ d'\lstinline'integer' vers \lstinline'int'
