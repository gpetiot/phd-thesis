...
respectant la grammaire de la figure~\ref{fig:gram-c}.
...
\begin{itemize}
\item \lstinline'p', \lstinline'p1', \lstinline'p2' et \lstinline'p3' sont des
  prédicats \eacsl;
\item \lstinline't', \lstinline't1', \lstinline't2' et \lstinline't3' sont des
  termes \eacsl;
\item $c$, $c_1$, $c_2$ et $c_3$ sont des fragments de programmes C;
\item \lstinline'e', \lstinline'e1', \lstinline'e2' et \lstinline'e3' sont des
  expressions C;
\item $l$, $l_1$, $l_2$, $l_3$, $Beg_f$, $End_f$, $BegIter_l$ et $EndIter_l$ sont
  des labels du programme;
\item \lstinline'i' et \lstinline'k' sont des identificateurs de variables liées
  dans un prédicat \eacsl et un compteur d'itérations dans un programme C;
\item \lstinline'x' est un identificateur de variable C;
...
terme (dont la grammaire est définie avec la figure~\ref{fig:gram-term}) peut
contenir des expressions C pures (sans effets de bord) et des mots-clés du
langage.
Par exemple, le mot-clé \lstinline{\result} permet de faire référence au
résultat d'une fonction, \lstinline{\valid} est un prédicat (sa grammaire est
définie en figure~\ref{fig:gram-pred}) qui renvoie vrai si son argument est un
pointeur valide (se référer au manuel du langage~\cite{\citeacsl}).
Les quantifications existentielles et universelles doivent porter sur des
...
Les invariants de boucle et les variants doivent être associés aux boucles.
