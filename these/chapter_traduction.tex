...
\commentAG{dans ce qui suit, garder ici ce qui explique l'exemple et reporter dans
les parties suivantes ce qui explique la traduction de ses annotations.}

 Le programme instrumenté obtenu après traduction des annotations de
cette fonction est présenté (utilisant la notation abrégée pour les entiers non
...
du listing ?? est présenté dans le listing~\ref{lst:abbrv}.
\commentAG{à déplacer plus loin :
``(utilisant la notation abrégée pour les entiers non bornés)''.}
Les principes de la traduction sont illustrés par des
extraits de ce listing, au fur et à mesure de leur présentation.
\commentAG{à faire comme indiqué.}

 La clause \lstinline'requires' de la ligne 1
(listing~\ref{lst:normalized}) indique que la taille du tableau \lstinline|n| est positive, elle est traduite
à la ligne 6 du listing~\ref{lst:abbrv}.
La clause \lstinline'requires' de la ligne 2 (listing~\ref{lst:normalized}) 
indique que \lstinline|(t+0)|, \ldots, \lstinline|(t+(n-1))| sont des pointeurs
valides, elle est traduite à la ligne 7 du listing~\ref{lst:abbrv}).
...
comme GMP.
Cette notation est utilisée aux lignes 12--13 du listing~\ref{lst:Z:1}.
