...
\begin{figure}[bt]
  \begin{center}
    \begin{tikzpicture}[>=latex,font=\sffamily]
      ...
      \node [anchor=west,memcell,full,minimum width=4cm] at (B.east) (C) {
        \begin{tikzpicture}
          \node[memcell,init] (p0) {[0]};
          \node[memcell,init,anchor=west] at (p0.east) (p1) {[1]};
          \node[memcell,uninit,anchor=west] at (p1.east) (p2) {[2]};
        \end{tikzpicture}
      };
      \node [anchor=west,memcell] at (C.east) (D) {};
      \node [anchor=west,memcell,full] at (D.east) (E) {$bloc_3$};
