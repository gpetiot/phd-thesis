...
Elle l'illustre sur l'exemple donné dans le listing~\ref{lst:normalized}.
...
\begin{myproperty}[Inclusion des environnements]
  \label{th:inclusion}
...
\begin{myproperty}[Traduction des Termes : Correction]
  ...
  \eval{\lstinline't'}{\env$_l$} = \eval{\lstinline'e'}{\env}, où
  \env = \comps{$A$}{\env$_l$}.
\end{myproperty}

\begin{myproperty}[Traduction des Termes : Absence d'erreur]
  \label{lem:term-noerror}
  \env $\neq$ \errorenv
\end{myproperty}

\begin{myproperty}[Traduction des Termes : Inclusion des environnements]
...
1. \eval{\lstinline'cst'}{\env} =
\eval{\lstinline'var_n'}{
  (\comps{$\Zinit$\underline{\lstinline'var_n = cst;'}}{\env})}.


2. \env{} \subenv{} \comps{$\Zinit$\underline{\lstinline'var_n = cst;'}}{\env}.


3. \comps{$\Zinit$\underline{\lstinline'var_n = cst;'}}{\env} $\neq$ \errorenv.
...
\begin{proof}[Justification du point 2.]~\\
  ...
  \env{} \subenv{}
  \env[\lstinline'var_n' $\mapsto$ \eval{\lstinline'cst'}{\env}] et donc :
  \env{} \subenv{} \comps{$\Zinit$\underline{\lstinline'var_n = cst;'}}{\env}.
  ~\\
\end{proof}
...
\begin{proof}[Justification du point 3.]~\\
  ...
  \env[\lstinline'var_n' $\mapsto$ \eval{\lstinline'cst'}{\env}], ce n'est pas
...
\begin{myproperty}[Traduction des Prédicats : Correction]
  ...
  $I$ et exécuté à partir d'un environnement quelconque \env$_l$ donne un
  environnement \env{} tel que
  \eval{\lstinline'p'}{\env$_l$} = \eval{\lstinline'e'}{\env}, où
  \env = \comps{$A$}{\env$_l$}.
\end{myproperty}

\begin{myproperty}[Traduction des Prédicats : Absence d'erreurs]
  \label{lem:pred-noerror}
  \env $\neq$ \errorenv
\end{myproperty}

\begin{myproperty}[Traduction des Prédicats : Inclusion des environnements]
...
produit si $\lnot$ \eval{\lstinline'p'}{(\comps{$A$}{\env})}.
...
\errorenv est produit si (\eval{\lstinline't'}{(\comps{$A$}{\env})}) $\ge$
...
si $\lnot$ \eval{\lstinline'p2'}{(\comps{$A$}{\env})}.
