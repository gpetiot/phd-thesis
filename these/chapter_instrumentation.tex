...
soient considérées ou que \lstinline'var_n' soit évaluée à $false$.
...
\lstinline'k' soient considérées ou que \lstinline'var_n' soit évaluée à $true$.
...
\begin{proof}[Justification de la propriété~\ref{th:error}]~\\
  ...
  (a) Cas où \eval{\lstinline'e'}{(\comps{$A$}{$\mem$})} = true, c'est-à-dire
  par hypothèse \eqlabel{h1} : \eval{\lstinline'p'}{$\mem$} = true.
  ...
  (b) Cas où \eval{\lstinline'e'}{(\comps{$A$}{$\mem$})} = false, c'est-à-dire
  par hypothèse \eqlabel{h1} : \eval{\lstinline'p'}{$\mem$} = false.
...
\begin{proof}[Justification de la propriété~\ref{th:inclusion}]~\\
  ...
  \comps{$A\semicolon~ \mbox{\lstinline'fassert(e);'}$}{$(\env, \store)$} par
  \comps{$A$}{$(\env, \store)$}, la propriété à prouver est rendue triviale
  grâce aux hypothèses \eqlabel{h4} et \eqlabel{h5}.
  ~\\
\end{proof}
