...
\comp{\lstinline'/*@ assert p; */ ;'}{$\mem$} = $\errormem$
si \comps{$A\semicolon~ \mbox{\lstinline'fassert(e);'}$}{$\mem$} = $\errormem$,
...
\comps{$A\semicolon~ \mbox{\lstinline'fassert(e);'}$}{$(\env, \store)$}.
...
\begin{proof}[Justification de la propriété~\ref{th:error}]~\\
  ...
  et prouvons que \comp{\lstinline'/*@ assert p; */ ;'}{$\mem$} = $\errormem$.

  (a) Cas où l'assertion est vraie, soit
  \eval{\lstinline'e'}{(\comps{$A$}{$\mem$})} = true, c'est-à-dire par hypothèse
  \eqlabel{h1} : \eval{\lstinline'p'}{$\mem$} = true.
  Développons \comps{$A\semicolon~ \mbox{\lstinline'fassert(e);'}$}{$\mem$} :
  ...
  \comps{$A$}{$\mem$} = $\errormem$,
  ce qui contredit l'hypothèse \eqlabel{h2} selon
  laquelle \comps{$A$}{$\mem$} $\neq \errormem$.

  (b) Cas où l'assertion est fausse, soit
  \eval{\lstinline'e'}{(\comps{$A$}{$\mem$})} = false,
  c'est-à-dire par hypothèse
  \eqlabel{h1} : \eval{\lstinline'p'}{$\mem$} = false.
  Développons \comp{\lstinline'/*@ assert p; */ ;'}{$\mem$} :
  ...
  Nous avons ainsi prouvé que si
  \comps{$A\semicolon~ \mbox{\lstinline'fassert(e);'}$}{$\mem$} = $\errormem$,\\
  alors \comp{\lstinline'/*@ assert p; */ ;'}{$\mem$} = $\errormem$.
  ~\\
\end{proof}
...
\begin{proof}[Justification de la propriété~\ref{th:inclusion}]~\\
  ...
  \begin{center}
    \begin{tabular}{lr}
      $\env \subenv \env''$ & \eqlabel{h1} \\
      $\store \subenv \store''$
      où $(\env'', \store'') = $ \comps{$A$}{$(\env, \store)$} & \eqlabel{h2} \\
    \end{tabular}
  \end{center}

  Nous avons montré (voir justification de la propriété~\ref{th:error}) que si
  l'assertion est valide, alors
  \comps{$A\semicolon~ \mbox{\lstinline'fassert(e);'}$}{$\mem$}
  = \comps{$A$}{$\mem$}.

  Prouver
  $\env \subenv \env'$ et $\store \subenv \store'$, avec $(\env', \store') =$
  \comps{$A\semicolon~ \mbox{\lstinline'fassert(e);'}$}{$(\env, \store)$}
  revient alors à prouver
  $\env \subenv \env''$ et $\store \subenv \store''$,
  avec $(\env'', \store'') =$ \comps{$A$}{$(\env, \store)$}, ce qui correspond
  aux hypothèses \eqlabel{h1} et \eqlabel{h2}.
  ~\\
\end{proof}
...
la mémoire d'erreur (cas \eqlabel{C-while-1}), ce qui correspond aux
...
obtient la mémoire d'erreur (cas \eqlabel{C-while-3}), ce qui correspond
...
assigns, alors la méoire d'erreur est obtenue (cas \eqlabel{f-2}), ce qui
...
la mémoire d'erreur (cas \eqlabel{f-3}), ce qui correspond aux lignes
...
la méoire d'erreur est obtenue (cas \eqlabel{f-1}), ce qui correspond aux
