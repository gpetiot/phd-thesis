...
La propriété~\ref{th:error} assure l'absence de faux positifs et l'absence de
...
\begin{proof}[Justification du point 1.]~\\
  ...
  soit \eval{\lstinline'e1'}{(\comps{$A_1$}{\env})} = true, c'est-à-dire par
  hypothèse \eqlabel{h1} : \eval{\lstinline'p1'}{\env} = true.
  ...
  soit \eval{\lstinline'e1'}{(\comps{$A_1$}{\env})} = false, c'est-à-dire par
  hypothèse \eqlabel{h1} : \eval{\lstinline'p1'}{\env} = false.
...
\begin{proof}[Justification du point 2.]~\\
  ...
  les hypothèses \eqlabel{h3}
  et \eqlabel{h4} nous assurent que les blocs $A_1$ et $A_2$ ne modifient pas
