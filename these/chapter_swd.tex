...
\begin{figure}[tb]
  \scriptsize{
    ...
    {\myinference{$\alpha$-assume-post}
      {(End_f, \mbox{\lstinline'p'}) \prule (I, \mbox{\lstinline'e'})}
...
\begin{figure}[tb]
  \scriptsize{
    ...
    {\myinference{$\alpha$-assume-invariant}
      {(l, \mbox{\lstinline'p'}) \prule (I, \mbox{\lstinline'e'})}
      {
        (l, \mbox{\lstinline'loop invariant p;'}) \arule
        I \concat (l, \mbox{\lstinline'fassume(e && !loopcond);'})
      }{}
    }

    %% {\myinference{$\alpha$-assume-invariant-2}
    %%   {(l, \mbox{\lstinline'p'}) \prule (I_1, \mbox{\lstinline'e1'}) \\
    %%     (EndIter_l, \mbox{\lstinline'p'}) \prule
