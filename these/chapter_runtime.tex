...
\While{$x$ n'est pas une feuille}
  \If{$\mathit{addr(x)} \Band \mathit{mask(x)} \neq a \Band \mathit{mask(x)}$}
    \Return $\mnull$
  \EndIf
  \If{$\mathit{addr(right(x))} \Band \mathit{mask(right(x))} == a \Band \mathit{mask(right(x))}$}
    \State $x \gets \mathit{right(x)}$
  \ElsIf{$\mathit{addr(left(x))} \Band \mathit{mask(left(x))} == a \Band \mathit{mask(left(x))}$}
    \State $x \gets \mathit{left(x)}$
  \Else
    \Return $\mnull$
  \EndIf
\EndWhile
\If{$a \neq \mathit{ptr(leaf(x))}$}
  \Return $\mnull$
\Else
  ...
  \While{$\mathit{true}$}
    \If{$x$ est une feuille}
      \If{$\mathit{addr(x)} > a$}
        ...
        \EndIf
      \ElsIf{$a < \mathit{size(leaf(x))} + \mathit{addr(x)}$}
        \Return $\mathit{leaf(x)}$
      \ElsIf{$\mathit{size(leaf(x))} == 0 \land a == \mathit{ptr(leaf(x))}$}
        \Return $\mathit{leaf(x)}$
      ...
      \EndIf
    \EndIf
    \If{$\mathit{addr(right(x))} \Band \mathit{mask(right(x))} \le a \Band \mathit{mask(right(x))}$}
      \State $\mathit{stack} \gets \mathit{push(left(x))}$
      \State $x \gets \mathit{right(x)}$
    \ElsIf{$\mathit{addr(left(x))} \Band \mathit{mask(left(x))} \le a \Band \mathit{mask(left(x))}$}
      \State $x \gets \mathit{left(x)}$
    \ElsIf{$\mathit{stack} =$ pile vide}
      \Return $\mnull$
    \Else
...
\State $x \gets \mathit{root}$ de type Patricia trie
\While{$x$ n'est pas une feuille}
  \If{$\mathit{addr(right(x))} \Band \mathit{mask(right(x))} == \mathit{ptr(b)} \Band \mathit{mask(right(x))}$}
    \State $x \gets \mathit{right(x)}$
  \Else
    \State $x \gets \mathit{left(x)}$
  \EndIf
\EndWhile
\If{$\mathit{father(x)} == \mnull$}
