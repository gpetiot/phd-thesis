...
\While{$tmp$ n'est pas une feuille}
  \If{$addr(tmp) \Band mask(tmp) \neq a \Band mask(tmp)$}
    \Return $NULL$
  \EndIf
  \If{$addr(right(tmp)) \Band mask(right(tmp)) == a \Band mask(right(tmp))$}
    \State tmp $\gets right(tmp)$
  \ElsIf{$addr(left(tmp)) \Band mask(left(tmp)) == a \Band mask(left(tmp))$}
...
\begin{algorithm}
\begin{algorithmic}
\If{$(root == NULL) \lor (a == NULL)$}
  \Return $NULL$
\Else
  \State tmp $\gets root$ de type Patricia trie
  \State stack $\gets$ pile vide
  \While{$true$}
    \If{$tmp$ est une feuille}
      \If{$addr(tmp) > a$}
        \If{$stack =$ pile vide}
          \Return $NULL$
        \Else
          ...
          \State continue
        \EndIf
      \ElsIf{$a < size(leaf(tmp)) + addr(tmp)$}
        \Return $leaf(tmp)$
      \ElsIf{$size(leaf(tmp)) == 0 \land a == ptr(leaf(tmp))$}
        \Return $leaf(tmp)$
      \ElsIf{$stack =$ pile vide}
        \Return $NULL$
      \Else
        \State tmp $\gets pop(stack)$
        \State continue
      \EndIf
    \EndIf
    \If{$addr(right(tmp)) \Band mask(right(tmp)) \le a \Band mask(right(tmp))$}
      \State stack $\gets push(left(tmp))$
      \State tmp $\gets right(tmp)$
    \ElsIf{$addr(left(tmp)) \Band mask(left(tmp)) \le a \Band mask(left(tmp))$}
      \State tmp $\gets left(tmp)$
    \ElsIf{$stack =$ pile vide}
      \Return $NULL$
    \Else
...
\While{$leaf$ n'est pas une feuille}
  \If{$addr(right(leaf)) \Band mask(right(leaf)) == ptr(b) \Band mask(right(leaf))$}
...
\Else
  ...
  \If{$brother$ n'est pas une feuille}
