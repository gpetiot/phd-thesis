...
surveiller les opérations bas niveau des programmes C.
Les générateurs de tests structurels pouvant rarement traiter les opérations
bas niveau, nous avons décidé de changer d'approche pour ces propriétés.
...
Dans un premier temps nous présentons les différentes annotations qui nous
intéressent (partie~\ref{sec:mem-annots}).
...
Détaillons maintenant les annotations \eacsl que nous considérons :
\lstinline{\base_addr}, \lstinline{\block_length},
\lstinline{\offset}, \lstinline{\valid}, \lstinline{\valid_read} et
\lstinline{\initialized}.
Les trois premières sont des termes \eacsl, les trois dernières sont des
prédicats \eacsl.

\lstinline'\base_addr(p)' retourne l'adresse de base du bloc alloué
qui contient le pointeur \lstinline{p}.
\lstinline'\block_length(p)' retourne la longueur (en octets) du
bloc alloué qui contient, le porinteur \lstinline{p}.
\lstinline'\offset(p)' retourne le décalage (en octets) entre \lstinline{p} et
son adresse de base.
\lstinline'\valid(p)' (respectivement \lstinline'\valid_read(p)') est vrai
si le déréférencement de \lstinline{p} est autorisé en
lecture et en écriture (respectivement au moins en lecture).
\lstinline'\valid(p)' implique \lstinline'\valid_read(p)'
mais l'inverse n'est pas vrai.
\lstinline'\initialized(p)' est un prédicat prenant un pointeur \lstinline{p}
sur une left-value en argument.
Ce prédicat est vrai si la left-value en question est initialisée.
...
\lstinline{\block_length} et \lstinline{\offset} qui sont des termes \eacsl.
On y voit un modèle mémoire simplifié contenant 3 blocs.
Le second bloc contient le pointeur \lstinline'p' à l'offset 1.
L'adresse de base de \lstinline'p' (\lstinline'\base_addr(p)') est indiquée au
début du bloc.
La longueur du bloc de \lstinline'p' (\lstinline'\block_length(p)') mesure la
taille du bloc à partir de son adresse de base.
L'offset de \lstinline'p' (\lstinline'\offset(p)') dans le bloc est 1.
...
instrumentées afin de pouvoir exploiter ces informations.
