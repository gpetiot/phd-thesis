...
traitons.
Nous définissons la notion d'environnement et de store ainsi que les fonctions
permettant de décrire la sémantique dénotationnelle de chaque élément du
langage.
...
Un environnement \env{} est une fonction totale \env :
$LV \rightarrow LOC \cup \{\bot\}$
...
\env[$\mathit{lv_1} \mapsto \loc_1, \mathit{lv_2} \mapsto \loc_2$] (en supposant
que $\mathit{lv_1} \neq \mathit{lv_2}$)
...
\begin{center}
\begin{tabular}{rclr}
  (\env[$\mathit{lv} \mapsto \loc$])$-~\{lv\}$ &=& \env & \eqlabel{env-del-1} \\
  (\env[$\mathit{lv_1} \mapsto \loc$])$-~\{\mathit{lv_2}\}$
...
La notation \env[$\loc \mapsto v$] désigne un nouveau store, correspondant au
...
\begin{center}
\begin{tabular}{rclr}
  (\store[$\loc \mapsto v$])$-~\{\loc\}$ &=& \store & \eqlabel{store-del-1} \\
  (\store[$\loc_1 \mapsto v$])$-~\{\loc_2\}$
...
Nous définissons un état d'erreur, noté \errorenv, de type $ENV \times STORE$.
...
Nous avons défini la notion d'environnement et de store, ainsi que la sémantique
