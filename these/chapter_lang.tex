...
corps de boucle.
Ceci n'est pas spécifié par la grammaire de la figure~\ref{fig:gram-c}.
...
Ces clauses sont facultatives.
Par exemple la fonction du listing~\ref{lst:normalized} n'a pas de clause
\lstinline'assigns'.
Les symboles non-terminaux \textit{exp}, \textit{term} et \textit{pred} ne sont
pas définis figure~\ref{fig:gram-c}.
Nous les définissons maintenant en partie~\ref{sec:gram-term}
et~\ref{sec:gram-pred}.
...
\textit{cst} est une constante entière ou une chaîne de caractère littérale.
Les constantes flottantes ne sont pas considérées.
Nous considérons ici \textit{cst} comme un symbole terminal.
...
types entiers bornés du C vers les entiers mathématiques.
...
\item $x$, $x_1$ et $x_N$ sont des identificateurs de variable;
...
\item $e$, $e_1$, $e_2$, $e_3$, $e_N$ sont des expressions C;
...
\item $\loc$, $\loc_1$, $\loc_2$, $\loc_N$ sont des adresses en mémoire;
...
Un environnement dénoté $\env$ est une fonction partielle
...
La fonction environnement est définie ainsi :
...
$(\mathit{lv}, \loc)$, si $\loc$ est l'adresse de $\mathit{lv}$
...
Le domaine des valeurs $VAL$ est défini comme l'union des valeurs des types que
...
$LOC$ est inclus dans $VAL$ car nous formalisons les pointeurs et la valeur
d'un pointeur est une adresse.
Un {\em store} est une fonction partielle $\store \in LOC \rightarrow VAL$ qui
associe une valeur à chaque adresse.
...
l'adresse $\loc$.
La fonction {\em store} est définie ainsi :
...
$(\loc, v)$, si $v$ est la valeur à l'adresse $\loc$ dans $\store_1$, alors
$v$ est également à l'adresse $\loc$ dans $\store_2$.
...
À une notation syntaxique $x\mbox{\lstinline'['}i\mbox{\lstinline']'}$ est
...
Cette fonction associe une valeur à une expression C (un terme et un prédicat
...
variable $k$ liée au terme $\mbox{\lstinline'\\lambda integer'}~k\semicolon~t_3$
est substituée dans le terme $t_3$ par une nouvelle variable $k'$ liée à
l'opérateur $\Sigma$ ou $\Pi$.
...
$\bot$) et différente de \lstinline'NULL'.
$\lnot p$ renvoie 1 si la valeur du prédicat $p$ est nulle, 0 sinon.
$p \land q$ renvoie 1 les valeurs des deux prédicats sont non-nulles, 0 sinon.
$p \lor q$ renvoie 1 si la valeur d'un des deux prédicats est non-nulle, 0
sinon.
$p \Rightarrow q$ renvoie 1 si la valeur de $p$ est nulle ou si la valeur de $q$
est non-nulle, 0 sinon.
$p \Leftrightarrow q$ renvoie 1 si $p$ et $q$ ont la même valeur, 0 sinon.
...
cette variable.
Ainsi, le prédicat \lstinline'\valid(x)' est vrai si \lstinline'x'
...
\lstinline'fassume', si c'est le cas on aboutit à une contradiction.
...
Nous appelons $G$ l'ensemble des left-values dans la portée courante, $G$
contient donc les left-values des variables globales, des paramètres formels de
la fonction courante, et des variables locales définies avant la boucle.
...
est toujours évitée grâce au \lstinline'variant'.
...
mémoire d'erreur $\errormem$ est obtenue si le prédicat $p_1$ s'évalue à
faux (\eqlabel{F-1}).
Si l'évaluation d'un élément appartenant à $G-X$ (ensemble des left-values
...
\eacsl en code C dont il justifie qu'il est sémantiquement équivalent, en
