...
d'environnement et de $store$, qui nous permettent de définir la notion de
...
Un environnement $\env$ est une fonction
$\env \in LV \rightarrow LOC \cup \{\bot\}$
qui associe une adresse ou $\bot$ (indéfini) à chaque left-value.
...
L'environnement $\env$[$\mathit{lv} \mapsto \loc$] est défini par :
...
$VAL = \cup~\mathbb{Z}~\cup~LOC~\cup~\{\bot\}$, où le symbole
...
Un store est une fonction $\store \in LOC \rightarrow VAL$ qui
...
Le store $\store$[$\loc \mapsto v$] est défini par :
...
$\store(\env(\mathit{x}$
\lstinline'['\eval{$\mathit{i}$}{$(\env, \store)$}\lstinline']'$))$.
...
Cette fonction associe une valeur à une expression C (et un terme et un prédicat
...
\lstinline'(x+t1)', ..., \lstinline'(x+t2)' peuvent être déréférencés et sont
...
élémént alloué ($\env(x$\lstinline'[0]'$)$), comme c'est le cas dans le langage
C.
