...
qui associe une adresse à chaque left-value.
...
\begin{tabular}{rclr}
  ...
  & \eqlabel{store-get-2}\\
  $\store$($\loc$) & = & $\bot$ si $\store$ est le store vide
  & \eqlabel{store-get-3} \\
\end{tabular}
\end{center}
...
Une mémoire $\mem$ est un paramètre et un résultat des fonctions
sémantiques définies dans la suite du chapitre.
$\mem = (\env, \store)$ sera noté :
\begin{itemize}
...
Nous définissons une mémoire spécifique d'erreur, notée $\errormem$.
...
de variable $x$, accédé à l'offset de valeur $e$.
