...
Appliquée à l’analyse de valeurs, elle consiste à calculer à chaque ligne du
code une sur-approximation de l’ensemble des valeurs prises par chaque variable
en cette ligne lors de toutes les exécutions du programme, permettant ainsi de
détecter certaines erreurs comme les divisions par zéro ou les accès en dehors
...
satisfaisabilité des $n$ prédicats.
...
critère ``tous les chemins'' impose une couverture de tous les chemins
faisables de $P$. L'exploration exhaustive de tous les chemins étant en pratique
irréalisable sur des programmes réels, le critère ``tous les k-chemins'' a
été défini.
...
des programmes C, conjointement dévelopée par Inria et le CEA LIST.
...
(\lstinline'ensures') assurées lors du retour.
...
\lstinline'b' ne produira pas d'erreur à l'exécution.
...
fonctions afin de décrire les états de la mémoire.
...
Ainsi, les postconditions (\lstinline'ensures') du listing~\ref{lst:swap}
signifient qu'à la fin de la fonction, \lstinline'*a' aura la valeur que
\lstinline'*b' avait au début de la fonction, et réciproquement.
...
donné du programme : les assertions (\lstinline'assert').
