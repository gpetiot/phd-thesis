...
dynamique qui va générer des invariants probables (inférence de la
spécification) \cite{discover-invariants}, une analyse statique qui va émettre
des alarmes concernant un non-respect des invariants, et une seconde analyse
dynamique qui va tenter de confirmer ces alarmes par une génération de tests
\cite{JCrasher}.\\

La première analyse dynamique utilise une génération de tests et des techniques
d'apprentissage pour générer des invariants. Ces invariants vont guider la
classification des alarmes et la génération de tests, ce qui rend l'outil très
dépendant leur qualité.
